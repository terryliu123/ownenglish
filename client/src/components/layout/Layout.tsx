      ...
      <div className="page-shell">
        ...
        {leftSidebar ? (
          <div className="mt-5 flex items-start gap-3">
            ...
            <main className="main-stage min-w-0 flex-1">{children}</main>
...
export function TeacherSidebar({
  ...
  const links: { id: string; label?: string; path?: string; icon?: string }[] = [
    { id: 'home', label: '首页', path: '/teacher', icon: 'H' },
  ...
  return (
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2 lg:hidden">
        {open ? (
          <nav className="min-w-[180px] rounded-2xl border border-gray-100 bg-white py-2 shadow-xl" style={{ animation: 'slideIn 0.2s ease-out' }}>
            {links.map((link) =>
              link.id === 'divider' ? (
                <div key="divider" className="my-1 mx-4 border-t border-gray-100" />
              ) : (
                <Link
                  key={link.id}
                  to={link.path!}
                  className={`flex items-center gap-3 px-4 py-3 text-sm font-medium transition-colors hover:bg-gray-50 ${
                    activePage === link.id ? 'text-coral' : 'text-gray-700'
                  }`}
