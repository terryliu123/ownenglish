...
## 2026-04-09 教师端非互动课堂页面骨架统一（第一轮）

### 涉及文件
- `client/src/components/layout/Layout.tsx`
- `client/src/components/layout/TeacherLeftSidebar.tsx`
- `client/src/components/layout/TeacherPageHeader.tsx`
...
- 新增教师端页面基础样式：
  ...
  - `teacher-control`
  - `teacher-stat-grid`
- 统一教师端浮动快捷导航和左侧导航中的硬编码乱码文案
- 任务组页面统一为：
  - 标准页头
  - 标准筛选/搜索工具条
  - 标准主内容卡片
- 课堂回顾页面去掉固定高度布局，改成统一页头、统计卡和筛选条
- 大屏互动页面改为统一页头结构，并保留原有配额与白板返回入口
...
### 回归重点
1. 任务组页面的页头、筛选条和按钮样式是否统一
2. 课堂回顾页面是否不再依赖固定高度且滚动正常
3. 大屏互动页头是否与教师端其它页面保持一致
4. 左侧导航和右下快捷导航文案是否正常可读

## 2026-04-09 教师端首页骨架与文案重写

### 涉及文件
- `client/src/pages/teacher/TeacherHome.tsx`

### 关键调整
- 教师首页改为使用统一页头组件 `TeacherPageHeader`
- 重写首页全部硬编码乱码文案，恢复为正式中文
- 保留原有数据逻辑：
  - 班级数量
  - 当前进行中的课堂
  - 最近课堂列表
...
### 回归重点
1. 教师首页标题、副标题和卡片文案是否为正常中文
2. 快速创建班级是否仍可正常创建并刷新数据
3. 最近课堂列表是否仍可正常跳转
4. 首页页头、卡片和按钮样式是否与任务组/课堂回顾/大屏互动页面保持一致

## 2026-04-09 班级页整页重写与统一样式收口

### 涉及文件
- `client/src/pages/teacher/Classes.tsx`

### 关键调整
- 班级页改为使用统一页头组件 `TeacherPageHeader`
- 重写班级页主要用户可见文案，去除硬编码乱码
- 保留原有班级管理能力：
  - 创建班级
  - 编辑班级
  - 删除班级
...
### 回归重点
1. 班级页标题、副标题、操作按钮是否为正常中文
2. 创建、编辑、删除、启停班级是否仍正常
3. 邀请码复制、加入链接复制、二维码预览与下载是否正常
4. 页头、卡片、弹窗按钮样式是否与首页/任务组/课堂回顾保持一致

## 2026-04-09 教具页整页重写与左侧菜单高度收口
...
### 关键调整
- 教具页改为使用统一页头组件 `TeacherPageHeader`
- 重写教具页主要用户可见文案，去除硬编码乱码
- 保留原有教具能力：
  - 公开教具库 / 我的教具
  - 搜索与分类筛选
  - 最近使用
...
### 回归重点
1. 左侧主菜单在常规分辨率下保持一屏高度，超出后内部可滚动
2. 教具页标题、副标题、标签页、按钮和弹窗文案是否为正常中文
3. 教具创建、编辑、删除、分享、打开预览是否仍正常
4. 教具页页头、卡片、按钮样式是否与首页/班级页/任务组页保持一致

## 2026-04-09 学生端首页与加入课堂页样式收口

### 涉及文件
- `client/src/pages/student/Home.tsx`
- `client/src/pages/student/JoinClass.tsx`
- `client/src/index.css`

### 关键调整
- 学生端首页重写为统一卡片式布局，去除主要用户可见乱码
- 加入课堂页重写为统一表单与引导样式，保留：
  - 已登录加入
  - 游客加入
  - 邀请码自动带入
  - 成功后跳转
...
### 回归重点
1. 学生首页标题、卡片、班级列表和游客提示是否为正常中文
2. 已登录加入班级流程是否正常
3. 游客加入班级流程是否正常
4. 学生首页和加入课堂页的卡片、按钮、输入框样式是否已统一

## 2026-04-09 教师端会员中心整页重写

### 涉及文件
- `client/src/pages/teacher/Membership.tsx`

### 关键调整
- 会员中心改为使用统一页头组件 `TeacherPageHeader`
- 重写会员中心主要用户可见文案，去除硬编码乱码
- 保留原有会员能力：
  - 当前会员状态展示
  - 额度使用情况
  - 会员方案对比
...
### 回归重点
1. 会员中心标题、副标题、方案卡片、订单记录是否为正常中文
2. 当前会员状态和额度展示是否正常
3. 创建订单、展示二维码、支付后自动刷新是否仍正常
4. 会员中心页头、卡片、按钮样式是否与首页/班级页/任务组页保持一致

## 2026-04-09 教师端导航角色收口
...
### 关键调整
- 明确左侧教师菜单为桌面端主导航
- 右下教师快捷菜单改为仅在小屏场景显示，避免与左侧主导航重复
- 左侧菜单文案恢复为正常中文
- 左侧菜单继续保持一屏高度、超出后内部滚动
- 顶部布局保留不变，减少导航层级冲突
